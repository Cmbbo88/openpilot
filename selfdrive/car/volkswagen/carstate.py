...
class CarState(CarStateBase):
  ...
  def update_pq(self, pt_cp, cam_cp, acc_cp, trans_type):
    ...
    if bool(pt_cp.vl["univACC"]['heartBeat']):
      ...
      self.univACCspeed = 0
      self.univbuttonState = pt_cp.vl["univACC"]['buttonState']
      ...
      ret.univACCenabled = False if (ret.cruiseState.enabled == False) else True if ((self.univbuttonState == 2 and not ret.univACCenabled) or (self.univACCtempdisable and not (ret.gasPressed or ret.brakePressed))) else False if (self.univbuttonState == 2 and ret.univACCenabled) else ret.univACCenabled
